...
def process_pred(pred_face, cls, thresh):
    ...
    mask_b = cv2.erode(mask_b, kernel_er, iterations=2)
...
def test_face(model, test_data, path, thresh, show, inv_trans=False):
    ...
    with torch.no_grad():
        for batch_idx, (x_test, y_test) in enumerate(test_data):
            ...
            gt_boxes, gt_labels = y_test["boxes"], y_test["labels"]
            gt_boxes, gt_labels = gt_boxes[0].numpy(), gt_labels[0].numpy()
            ...
            if show["images"]:
                if show["mask_seg"]:
                    ...
                    seg = np.zeros((pred_face.shape[0], pred_face.shape[1], 3)).astype('uint8')
                    seg[:,:,-1] = mask_seg
                    cv2_imshow(0.6*x_test+0.4*seg*255)
    ...
    return pred_boxs_per_image, true_boxs_per_image
